...
public class BDBJE
{
        ...
        if ( coreEnv == null )
        {
            try
            {
                ...
                if ( this.attributes.getCachePercent() != -1 )
                {
                    ...
                    if ( log.isDebugEnabled() )
                    {
                        log.debug( "Set JE CachePercent to '" + this.attributes.getCachePercent() + "'" );
                    }
                }
    ...
    private synchronized void init()
    {
        ...
        if ( coreDb == null )
        {
            ...
            try
            {
                ...
                coreDb = coreEnv.openDatabase( txn, attributes.getCacheName(), dbConfig );
                if ( log.isInfoEnabled() )
                {
                    log.info( "created db for region = '" + attributes.getCacheName() + "'" );
                }
                txn.commit();
        ...
        throws IOException
    {
        ...
        try
        {
            ...
            EntryBinding keyBinding = new SerialBinding( catalog, key.getClass() );

            /*
             * Create a serial binding for MyData data objects. Serial bindings
             * can be used to store any Serializable object.
             */
            EntryBinding dataBinding = new SerialBinding( catalog, ICacheElement.class );
            ...
            OperationStatus retVal = coreDb.get( null, searchKey, foundData, LockMode.DEFAULT );
        ...
        throws IOException
    {
        try
        {
             ...
             */
            EntryBinding dataBinding = new SerialBinding( catalog, ICacheElement.class );
            EntryBinding keyBinding = new SerialBinding( catalog, item.getKey().getClass() );
             ...
             * conditions are found such as deadlock. However, the status return
    ...
    protected synchronized void dispose()
    {
        ...
        if ( coreEnv != null & coreDb != null )
        {
            ...
            if ( databases.size() == 0 )
            {
                 ...
                 * (DatabaseException dbe) { log.error("Error synching coreEnv: " +
                 * dbe.toString()); }
                 */
                // close environment
                try
